...
class HueyExecutionLog(models.Model):
    ...
    def _reached_max_tries(cls, code, max_tries):
        failed_tries = HueyExecutionLog.objects.filter(
            code=code
        ).order_by("-start_time")[:max_tries]
        if failed_tries.count() < max_tries:
            return False
        fails = [
            not i
            for i in failed_tries.values_list("is_success", flat=True)
        ]
        return all(fails)
        ...
        def _decorator(func):
            def _inner_function(*args, **kwargs):
                ...
                if HueyExecutionLog._reached_max_tries(code, max_tries):
                    last_execution = HueyExecutionLog.objects.filter(
                        code=code
                    ).only("start_time").order_by(
                        "-start_time"
                    ).first().start_time
                    minutes = ((datetime.now() - last_execution).seconds / 60.0)
